  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        <div>
          <h1 className="font-playfair text-2xl md:text-3xl font-bold text-[#3D0026]">Settings</h1>
          <p className="text-sm text-[#C2185B]/60 mt-0.5">Manage your profile and preferences</p>
        </div>

        {/* Google account info */}
        {session?.user && (
          <div className="bg-white rounded-2xl border border-pink-100 p-5 card-shadow">
            <h2 className="font-playfair text-base font-semibold text-[#3D0026] mb-4 flex items-center gap-2">
              <span className="text-base">🔗</span> Google Account
            </h2>
            <div className="flex items-center gap-3">
              {session.user.image && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={session.user.image} alt="" className="w-10 h-10 rounded-full ring-2 ring-pink-100" />
              )}
              <div>
                <p className="text-sm font-semibold text-[#3D0026]">{session.user.name}</p>
                <p className="text-xs text-gray-400">{session.user.email}</p>
              </div>
              <span className="ml-auto text-xs bg-green-50 text-green-600 border border-green-100 px-2.5 py-1 rounded-full font-medium">
                ✓ Connected
              </span>
            </div>
          </div>
